...
def packet_handler (event):
   ...
   # Random selection of servers
    iterator = random.randint(0,count_server-1)
    print iterator
    if(not IPAddr(message.match.nw_src) in clients):
	...
	clients[IPAddr(message.match.nw_src)] = iterator
    else:
    ...
    reply.match.new_dest = message.match.nw_src
...
def launch ():
    ...
    log.info("Running Stateful Random Technique Load Balancer ")
